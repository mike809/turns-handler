...
namespace TurneroCentroMED.Entidades
{
    static class Funciones
    {
        ...
        public static int BuscarIndiceToRemove(ListaEstudios tmpEstudios ,Estudio _estudio)
        {
            for (int i = 0; i < tmpEstudios.Count; i++)
            {
                if (tmpEstudios[i].EstudioId == _estudio.EstudioId)
                {
                    return i;
                }
            }
