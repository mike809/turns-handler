            ...
            while (this.Count != 0)
            {
                ...
                for (j = 0; j < temporal.Count;)
                {
                    if ((temporal[j].Hora_de_turno.AddMinutes(temporal[j].EstudioDuracion)) < DateTime.Now)
                    {
                        ...
                        //cuando se le borre un estudio del departamento, se le borra al paciente tambien.
                        int x = GetPatient(temporal[j].IdPaciente);
                        if (x > -1)
                        {
                            temporal.Remove(temporal[j]);
                            if (FrmPrincipal.Pacientes[x].Horario.Count == 1)
                            {
                                FrmPrincipal.Pacientes.Remove(FrmPrincipal.Pacientes[x]);
                            }
                            //else
                            //{
                              //  int _index = Funciones.BuscarIndiceToRemove(FrmPrincipal.Pacientes[x].Horario, temporal[j]);
                                //if ( _index != -1)
                                //{
                                   // FrmPrincipal.Pacientes[x].Horario.Remove(FrmPrincipal.Pacientes[x].Horario[_index]);
                                //}
                        
                                //FrmPrincipal.Pacientes[x].Horario.Remove(temporal[j]);
                            //Esto debe estar fuera de aqui preguntando si el proximo paciente tiene su turno ahora. Puede que haya un hueco en el horario
                            //y el siguiente paciente no tenga su turno inmediatamente despues de el que paso.

                            //FrmPrincipal.reader.SpeakAsync(
                            //   string.Format("Patient number {0} Please go to the department of {1} For your test",
                            //               temporal[j].IdPaciente, temporal[j].EstudioDept));
                            //} 
                        }                 
